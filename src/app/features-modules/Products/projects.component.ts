...
export class ProjectsComponent {
  ...
  typeProductFilters = [
    ...
    { name: 'Hotte de cuisine', selected: false },
...
@ViewChild('categoryModal') categoryModal!: ElementRef;
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      //this.openModal(); this is the category thing
    }, 100);
    AOS.init({
      duration: 1200,
      once: true,
  ...
  applyFilter() {
    ...
    this.filteredArticles = this.filteredArticlestrue.filter(article => {
        return this.materialFilters.every(material => {
        ...
        }) && this.industryFilters.every(industry => {
        ...
        }) && this.typeProductFilters.every(typeProduct => {
